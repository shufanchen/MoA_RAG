...
from ragas.metrics import answer_similarity,faithfulness,answer_correctness,answer_relevancy, noise_sensitivity_relevant, noise_sensitivity_irrelevant,summarization_score
import datetime
import warnings
# 忽略警告
...
def read_jsonl(file_path):
    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            #data_samples['summary'].append(sample.get('answer'))
...
def process_evaluation(file_path):
    ...
    data_samples = read_jsonl(file_path)
    ...
    result = evaluate(
        dataset,
        #metrics=[answer_correctness, answer_similarity, faithfulness, answer_relevancy, noise_sensitivity_relevant, noise_sensitivity_irrelevant, summarization_score],
    ...
    output_csv_path = os.path.join(base_dir, f'{model_name}_eval_result.csv')
